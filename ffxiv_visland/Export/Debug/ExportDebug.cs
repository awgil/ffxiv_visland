...
public unsafe class ExportDebug {
    ...
    public void Draw() {
        ...
        foreach (var n1 in _tree.Node($"Agent: {(nint)agent:X}", agent == null && agent->Data != null)) {
            ...
            _tree.LeafNode($"Currency 0: {agent->Data->CurrencyItemIds[0]} '{Item.GetRow(agent->Data->CurrencyItemIds[0])?.Name}': {agent->Data->CurrencyCounts[0]}/{agent->Data->CurrencyStackSizes[0]}");
            _tree.LeafNode($"Currency 1: {agent->Data->CurrencyItemIds[1]} '{Item.GetRow(agent->Data->CurrencyItemIds[1])?.Name}': {agent->Data->CurrencyCounts[1]}/{agent->Data->CurrencyStackSizes[1]}");
